...
数据层防护
各模块数据物理隔离存储，避免跨业务数据泄露
代码、文档、附件全量加密持久化存储
加密传输协议，保障数据传输链路安全
应用层权限管控
平台全局统一总权限管控
每个功能模块独立精细化访问权限分配
完整用户操作日志留存，所有行为可追溯审计
🏗️ 底层技术架构
核心架构：微服务 + K8S 容器化云原生部署，弹性扩容、高可用、易维护
存储组件：PostgreSQL 关系库、时序数据库、Chroma 向量数据库、分布式缓存
消息中间件：RabbitMQ，负责异步通知、任务调度
AI 底层能力：文档切片、向量嵌入、大模型推理、RAG 检索增强生成
部署方案：整体一键部署 / 单一模块独立拆分部署
🔌 开放集成能力
提供完整对外接口，支持企业二次对接开发：
全功能 RESTful Open API
Webhook 事件回调，打通外部业务系统
OAuth2 第三方登录鉴权
前端插槽自定义，拓展平台页面功能
📦 两种部署方案
私有化部署（企业客户推荐）
部署在企业自有服务器或专属隔离云，所有业务、代码、文档数据留存企业内部，完全自主掌控数据；支持按需单独部署某一个模块。
SaaS 云端版本
开箱即用，算力按需付费，轻量化适配中小研发团队。
✨ 产品核心优势
全场景无缝协同
一套平台覆盖编码、项目、文档、存储、沟通、AI 六大研发环节，无需切换多款第三方工具，大幅降低团队沟通成本。
企业数据与知识完全可控
双层安全防护 + 私有化部署方案，实现团队技术知识长期沉淀复用，杜绝数据外泄风险。
全方位提升研发效率
秒级开发环境搭建、全链路 AI 辅助、自动化敏捷流程，缩短项目交付周期，降低人力成本。
高拓展兼容能力
插件化架构 + 微服务拆分，适配独立开发者、小型团队、大型集团多类研发场景。
🖥️ 全终端适配
<div align="center">
<img src="./assets/pc-screen.png" width="400" alt="PC端界面">
<img src="./assets/mobile-app.png" width="220" alt="手机APP界面">
<p style="font-size:14px; color:#666;">全终端统一操作体验：网页端 / PC客户端 / 移动端APP / 微信小程序</p>
</div>
所有功能多端数据实时同步：
Web 浏览器网页端
Windows/Mac PC 客户端
...
微信小程序
📌 快速上手流程
部署 AT Work 镜像（SaaS 开通使用 / 容器私有化部署）
创建团队工作空间，配置成员访问权限
Coding 模块：选择技术模板 5 秒创建云开发环境，开启 AI 代码助手
Task 模块：录入业务需求，规划迭代周期，拆解跟踪开发任务
Wiki + Drawer：沉淀项目文档、统一管理研发资产文件
思链 AI：导入企业内部资料，开启全平台 AI 辅助能力
IM 嗷嗷：团队实时沟通、远程线上会议协同
📄 授权说明
